...
// Update form: secretKey is optional — leave blank to keep the existing key
// (the GET response never returns secretKey, so it cannot be pre-filled).
export const updateMarketplaceAccountSchema = z.object({
  platform: z.string()
    .min(1, '플랫폼을 입력하세요')
    .max(50, '플랫폼은 50자 이하여야 합니다'),
  accountAlias: z.string()
