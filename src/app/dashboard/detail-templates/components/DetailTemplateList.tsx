...
export function DetailTemplateList() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setError('');
      try {
        const list = await useCase.listTemplates();
        if (!alive) return;
        setTemplates(list);
      } catch {
            ...
            <tbody>
              {templates.map((t) => (
                <tr
                  ...
                  onClick={() => router.push(ROUTES.DETAIL_TEMPLATE_EDIT(t.id))}
                >
                  ...
                  <td className="px-4 py-3 text-gray-700">{t.blockCount ?? t.blocks?.length ?? 0}</td>
                  <td className="px-4 py-3">
                    <button
                      type="button"
                      onClick={(e) => {
