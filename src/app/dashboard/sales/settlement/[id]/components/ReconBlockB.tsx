 ...
 *
 * ⚠️ 부호 규칙: 서버의 `totalDiff = 실정산 − 예상`(통장 기준)이라 <b>양수 = 예상보다 더 받음</b>이다.
 * 헤더에 이 규칙을 적어 두지 않으면 숫자를 반대로 읽는다 — 실제로 이 주석과 헤더 문구가 반대로 적혀 있었다.
 * 근거는 서버의 `SettlementDiffAnalyzer`(`actualTotal − expectedTotal`)와 `SettlementReconciler`
 * (`finalAmount − ourTotal`)다. 화면 전체가 이 한 가지 부호만 쓴다.
          ...
          <div className="space-y-1">
            ...
            <p className="text-sm text-gray-500">
              {unmatchedView ? (
                ...
                <>
                  판매 건별 원인 분해입니다. 차액은 <b>실정산 − 예상</b>이라 양수면 예상보다 더 받은
                  금액입니다.
                </>
              )}
            </p>
