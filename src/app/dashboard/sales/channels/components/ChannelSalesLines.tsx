 ...
 * ⚠️ 합계 줄을 만들지 않는다 — 위 채널 행이 서버가 계산한 합계다. 화면에서 다시 더하면 두 숫자가 갈린다.
 */
  ...
  return (
    <table className="w-full text-xs">
      ...
      <tbody className="divide-y divide-gray-200">
        {rows.map((row) => (
          <tr key={`${row.masterProductId ?? 'uncategorized'}-${row.accountId ?? 'all'}`}>
            ...
            <td className="px-2 py-1 text-right text-gray-900">{formatMoney(row.grossSales)}</td>
            <td className="px-2 py-1 text-right text-gray-500">{formatMoney(row.discount)}</td>
            <td className="px-2 py-1 text-right text-gray-500">{formatMoney(row.estFee)}</td>
            <td
              className="px-2 py-1 text-right text-gray-900"
